...
class Navigator:
    ...
    def __init__(self, swarmPopulation):
        self.swarmPopulation = swarmPopulation
        rospy.init_node('nav_node', anonymous=True)
        print("Hi I am __init__ from nav_node")
    ...
    def starting_point_func(self):
        ...
        for i in range(self.swarmPopulation):
    ...
    def next_step_to_target(self, goal_coords):
        ...
        for i in range(self.swarmPopulation):
            ...
            next_step_Service.append(rospy.Service(next_step_service_for_UAV[i], next_step, self.next_step_handler))
            rospy.spin()
            next_step_Service[i].shutdown()
