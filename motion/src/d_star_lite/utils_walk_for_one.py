    ...
    for y_divider in range(1, attempted_on_patrol_population):
        for x_divider in range(y_divider, y_divider+2):
            if x_divider*y_divider is attempted_on_patrol_population:
                ...
                diff1 = attempted_on_patrol_population - (previous_divider["x"]*previous_divider["y"])
                ...
                if diff1 > diff2 and attempted_on_patrol_population != swarmPopulation: # only when there are UAVs on detect mode
                    ...
                    x_divider = previous_divider["x"]
                    y_divider = previous_divider["y"]
                    breaker = True
                    break
            previous_divider["x"] = x_divider
        if breaker:
            break
    ...
    for id in range(swarmPopulation):
        if id != ID:
            ...
            for i in range(1,-1, -1):
                for row in range( two_last_waypoints[ID][i][1] - obstacleRange, two_last_waypoints[ID][i][1] + obstacleRange + 1):
                    for column in range( two_last_waypoints[ID][i][0] - obstacleRange, two_last_waypoints[ID][i][0] + obstacleRange + 1):
...
def clear_UAV_obs_cells (swarmPopulation, ID, graph_for_UAV, two_last_waypoints, X_DIM, Y_DIM):
    ...
    for id in range(swarmPopulation):
        if id != ID:
            for row in range( two_last_waypoints[ID][0][1] - obstacleRange, two_last_waypoints[ID][0][1] + obstacleRange + 1):
                for column in range( two_last_waypoints[ID][0][0] - obstacleRange, two_last_waypoints[ID][0][0] + obstacleRange + 1):
                    if column in range(X_DIM) and row in range(Y_DIM): # if given cell doesn't surpass map dimensions                            
                        graph_for_UAV[id].cells[row][column] = 0
