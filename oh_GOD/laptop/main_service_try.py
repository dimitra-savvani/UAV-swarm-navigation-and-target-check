...
from motion.srv import ros_to_Dstar, ros_to_DstarRequest
...
def starting_point_cb(starting_point, args):
    ID = args[0]
    starting_point_for_UAV[ID] = starting_point,args.data
    print("starting_point_cb for uav", ID)
...
if __name__ == "__main__":
    ...
    for i in range(swarmPopulation):
        pygame_position_service =  "uav" + str(i) + "/motion/pygame/position"
        #starting_point_for_UAV.append('none')
        #starting_point_for_UAV.append(rospy.wait_for_message(pygame_position_service, String, timeout=None))
        rospy.wait_for_service(pygame_position_service)
        try:
            ros_to_Dstar_handler = rospy.ServiceProxy(pygame_position_service, ros_to_Dstar)
            s = ros_to_DstarRequest()
            s.request = True
            starting_point = ros_to_Dstar_handler(s) 
            starting_point_for_UAV.append(starting_point.position)
        except rospy.ServiceException as e:
            print("Service call failed: %s"%e)
        # rospy.Subscriber(pygame_position_service, String, starting_point_cb, (i))
        rospy.spin()
    ...
    pos_coords = []
    for i in range(swarmPopulation):
        graph_for_UAV[i].setStart(starting_point_for_UAV[i].data,) # set initial UAV position for each drone, to its graph
        ...
        graph_for_UAV[i], queue[i], k_m[i] = initDStarLite(graph_for_UAV[i], queue[i], starting_point_for_UAV[i].data, target_point, k_m[i])

        current_position_for_UAV.append(starting_point_for_UAV[i].data)
